...
[`api_tests/test_users_api.py`](../api_tests/test_users_api.py) — status
codes, response schema, response time, and header behavior — kept as an
actual Postman artifact rather than folded into the pytest suite. This is
the tool used for manual/exploratory API regression day-to-day (see the
main [README](../README.md)); the pytest suite is the automated,
